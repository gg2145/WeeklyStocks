...
class FixedMonitor(QDialog):
    ...
    def connect_ibkr(self):
        ...
        try:
            ...
            # Use a different client ID for monitor to avoid conflicts
            try_client_id = client_id + 10  # Use config client ID + 10 to avoid conflicts
            self.log_msg(f"🔍 Using monitor client ID {try_client_id} (base {client_id} + 10 to avoid conflicts)")
    ...
    def refresh_data(self):
        ...
        try:
            ...
            self.log_msg(f"🔍 DEBUG: Got {len(tracked_pending_sales)} pending sales, {len(tracked_pending_buys)} pending buys")
            self.log_msg(f"🔍 DEBUG: Position symbols: {list(positions.keys())}")
            ...
            self.log_msg(f"🔍 DEBUG: Combined symbols to display: {sorted(all_symbols)}")
            self.log_msg(f"🔍 DEBUG: all_symbols length: {len(all_symbols)}")
            
            if len(all_symbols) == 0:
                self.log_msg("❌ ERROR: No symbols to display! This is why the loop doesn't run!")
                return
                
            self.table.setRowCount(len(all_symbols))
            self.log_msg(f"🔍 DEBUG: Set table row count to {len(all_symbols)}")
            row = 0
            
            for symbol in sorted(all_symbols):
                try:
                    self.log_msg(f"🔍 DEBUG: Processing {symbol}")
                    ...
                    self.log_msg(f"🔍 DEBUG: {symbol} - pending_sells={has_pending_sells}, pending_buys={has_pending_buys}")
    ...
    def add_buy_order(self):
        """Add a buy order for a new symbol"""
        if not self.ib or not self.ib.isConnected():
            self.log_msg("❌ Not connected to IBKR - cannot place buy order")
            return
            
        # Get symbol
        symbol, ok1 = QInputDialog.getText(
            self, 'Add Buy Order',
        ...
        # Confirm order
        reply = QMessageBox.question(
            self, 'Place Buy Order',
            f'Place MARKET BUY order for {qty} shares of {symbol}?\n\n'
            f'This will:\n'
            f'1. Place MARKET buy order in IBKR immediately\n'
            f'2. Mark as PENDING BUY in our system\n'
            f'3. Show in monitor with BLUE status',
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        
        if reply == QMessageBox.StandardButton.Yes:
            try:
                # Create contract and place IBKR buy order
                contract = Stock(symbol, 'SMART', 'USD')
                order = MarketOrder('BUY', qty)
                
                self.log_msg(f"🔄 Placing MARKET buy order: {qty} shares of {symbol}")
                
                # Place the order in IBKR
                trade = self.ib.placeOrder(contract, order)
                
                # Track the pending buy immediately
                result = pending_tracker.mark_as_pending_buy(
                    symbol, qty, "MARKET", 
                    notes=f"New buy order via monitor at {datetime.now().strftime('%H:%M:%S')}"
                )
                
                self.log_msg(f"✅ IBKR buy order placed: {qty} shares of {symbol}")
                self.log_msg(f"✅ {result}")
                
                # Refresh display to show the new pending buy
                QTimer.singleShot(1000, self.refresh_data)
                
            except Exception as e:
                self.log_msg(f"❌ Error placing buy order for {symbol}: {e}")
                import traceback
                self.log_msg(f"❌ Traceback: {traceback.format_exc()}")

    def buy_position_with_tracking(self, symbol):
        """Buy position with actual IBKR order placement and tracking"""
        if not self.ib or not self.ib.isConnected():
            self.log_msg("❌ Not connected to IBKR - cannot place buy order")
            return
